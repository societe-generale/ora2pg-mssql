...
use vars qw($VERSION %OBJECT_SCORE $SIZE_SCORE $FCT_TEST_SCORE $QUERY_TEST_SCORE %UNCOVERED_SCORE %UNCOVERED_MYSQL_SCORE %UNCOVERED_SQLSERVER_SCORE @ORA_FUNCTIONS @MYSQL_SPATIAL_FCT @MYSQL_FUNCTIONS @SQLSERVER_FUNCTIONS %EXCEPTION_MAP);
...
# List taken from https://docs.microsoft.com/en-us/sql/t-sql/functions/functions?view=sql-server-2017
# TODO: Weight should be given to several functions later on
@SQLSERVER_FUNCTIONS = (
...
sub convert_plsql_code
{
	...
	for (my $i = 0; $i <= $#code_parts; $i++) {
		...
		# Things that must be done when functions are replaced with placeholder
		...
		foreach my $k (keys %{$class->{single_fct_call}}) {
			...
			if ($class->{single_fct_call}{$k} =~ /^CAST\s*\(/i) {
				...
				} elsif ($class->{is_sqlserver}) {
					$class->{single_fct_call}{$1} = Ora2Pg::SQLServer::replace_sql_type($class->{single_fct_call}{$1}, $class->{pg_numeric_type}, $class->{default_numeric}, $class->{pg_integer_type}, %data_type);
				} else {
...
sub plsql_to_plpgsql
{
	...
	return Ora2Pg::SQLServer::sqlserver_to_plpgsql($class, $str) if ($class->{is_sqlserver});
	...
	if ($class->{is_mysql}) {
		...
		$str =~ s/(BEGIN.*?DECLARE\s+)(.*?)(\s+BEGIN)/$1 . Ora2Pg::SQLServer::replace_sql_type($2, $class->{pg_numeric_type}, $class->{default_numeric}, $class->{pg_integer_type}, %data_type) . $3/iges;
	} else {
...
sub replace_oracle_function
{
	...
	if ($class->{is_sqlserver}) {
		$str =  Ora2Pg::SQLServer::sqlserver_to_plpgsql($class, $str);
	}
...
sub estimate_cost
{
	...
	return sqlserver_estimate_cost($str, $type) if ($class->{is_sqlserver});
...
sub sqlserver_estimate_cost
{
	...
	foreach my $t (keys %UNCOVERED_SQLSERVER_SCORE) {
		$cost += $UNCOVERED_SQLSERVER_SCORE{$t}*$cost_details{$t};
	}
	foreach my $f (@SQLSERVER_FUNCTIONS) {
		if ($str =~ /\b$f\b/igs) {
			$cost += 2;
			$cost_details{$f} += 2;
		}
