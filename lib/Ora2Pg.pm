...
sub _init
{
	...
	foreach my $k (keys %options) {
		...
		} elsif (($k eq 'is_sqlserver') && $options{is_sqlserver}) {
			$self->{is_sqlserver} = $options{is_sqlserver};
		} elsif ($options{$k} ne '') {
	...
	# Autodetect if we are exporting a MySQL database
	if ($self->{oracle_dsn} =~ /dbi:mysql/i) {
		$self->{is_mysql} = 1;
	}
	# Or a SQL Server database
	if ($self->{oracle_dsn} =~ /dbi:ODBC/i) {
		$self->{is_sqlserver} = 1;
	}
	...
	if ($self->{data_type}) {
		...
		elsif ($self->{is_sqlserver}) {
			%{$self->{data_type}} = %Ora2Pg::SQLServer::SQLSERVER_TYPE;
		}
	...
	if (!$self->{input_file}) {
		...
		elsif ($self->{oracle_dsn} =~ /dbi:ODBC/i) {
			$self->{dbh} = $self->_sqlserver_connection();
...
sub _sqlserver_connection
{
	my ($self, $quiet) = @_;

	use Ora2Pg::SQLServer;

	$self->logit("Trying to connect to database: $self->{oracle_dsn}\n", 1) if (!$quiet);

	# my $dbh = DBI->connect("$self->{oracle_dsn}", $self->{oracle_user}, $self->{oracle_pwd});
	# print "ORACLE_DSN = $self->{oracle_dsn}.\n";
	my $dbh = DBI->connect($self->{oracle_dsn}, $self->{oracle_user}, $self->{oracle_pwd},
		{
			ora_envhp => 0,
			LongReadLen=>$self->{longreadlen},
	...
	# Instruct Ora2Pg that the database engine is SQL Server
	$self->{is_sqlserver} = 1;
...
sub _packages
{
	...
	$self->{packages} = $self->_get_packages() if(!$self->{is_mysql} && !$self->{is_sqlserver});
...
sub _tables
{
	...
	if (scalar keys %tables_infos > 0) {
		...
		# print "lib\check_constraint\n";
		if (!$self->{skip_checks} && $self->{is_sqlserver}) {
			# TODO: future placeholder ?
			my %check_constraints = Ora2Pg::SQLServer::_check_constraint($self,$self->{schema});
			foreach my $tb (keys %check_constraints) {
				next if (!exists $tables_infos{$tb});
				%{$self->{tables}{$tb}{check_constraint}} = ( %{$check_constraints{$tb}});
			}
		}
		elsif (!$self->{skip_checks} && !$self->{is_mysql}) {
	...
	foreach my $t (sort keys %tables_infos) {
		...
		# if ($self->{is_sqlserver}) {
		# 	$tmp_tbname = $self->{schema} . "." . $tmp_tbname;
		# }
	...
	elsif (!$self->{is_mysql} && ($self->{db_version} !~ /Release 8/)) {
...
sub _directories
{
	...
	%{$self->{directory}} = $self->_get_directory() if(!$self->{is_mysql} && !$self->{is_sqlserver}); 
}
		...
		foreach my $view (sort sort_view_by_iter keys %ordered_views) {
			...
			if (!$self->{disable_comment}) {
				...
				foreach $f (sort keys %{$self->{views}{$view}{column_comments}}) {
					...
					$sql_output .= "COMMENT ON COLUMN " . $self->quote_object_name("$tmpv") . "." . $self->quote_object_name("$fname")       # test ANTHONY
...
LANGUAGE plpgsql ;
	...
	if ($self->{type} eq 'SEQUENCE') {
		...
		foreach my $seq (sort { $a->[0] cmp $b->[0] } @{$self->{sequences}}) {
			...
			if($self->{is_sqlserver}){
				$sql_output .=" OWNED BY ".$seq->[8].'.'.$seq->[9]
			}
			$sql_output .= "$cycle;\n";
			if($self->{is_sqlserver}) {

			    $sql_output .="ALTER TABLE ".$seq->[8]." ALTER COLUMN ".$seq->[9]." SET DEFAULT nextval('". $self->quote_object_name($seq->[0])."');\n";
			}
					...
					if ($self->{plsql_pgsql}) {
						...
						if ($#parts > 0) {
							if ($self->{is_mysql}) {
								...
								$parts[0] = Ora2Pg::SQLServer::replace_sql_type($parts[0], $self->{pg_numeric_type}, $self->{default_numeric}, $self->{pg_integer_type}, %{$self->{data_type}});
							} else {
...
-------------------------------------------------------------------------------
	...
	foreach my $table (sort {
		...
		# print "DUMPING TABLE 1\n";
		if (exists $self->{tables}{$table}{column_info}) {
			# print "DUMPING TABLE 2\n";
			my $schem = '';
			...
			foreach my $k (sort { 
				...
				$f->[2] =~ s/\D//g if (($f->[2] != -1) && ($self->{is_sqlserver}));          # for SQL Server, -1 means "max" for several data types 
		...
		if (!$self->{disable_comment}) {
			foreach $f (keys %{$self->{tables}{$table}{column_comments}}) {
				...
				$sql_output .= "COMMENT ON COLUMN " .  $self->quote_object_name("$tbname") . "." . $self->quote_object_name("$fname") . " IS E'" . $self->{tables}{$table}{column_comments}{$f} .  "';\n"; 
...
sub _column_comments
{
	...
	return Ora2Pg::SQLServer::_column_comments($self, $table) if ($self->{is_sqlserver});
...
sub _extract_sequence_info
{
	...
	return Ora2Pg::SQLServer::_extract_sequence_info($self) if ($self->{is_sqlserver});
...
sub _sql_type
{
	...
	# print "SQLTYPE:type=[$type], len=[$len], precision=[$precision] \n";
	return Ora2Pg::MySQL::_sql_type($self, $type, $len, $precision, $scale) if ($self->{is_mysql});
	return Ora2Pg::SQLServer::_sql_type($self, $type, $len, $precision, $scale) if ($self->{is_sqlserver});
...
sub _column_info
{
	...
	# return Ora2Pg::SQLServer::_column_info($self,'',$owner,'TABLE') if ($self->{is_sqlserver});
	return Ora2Pg::SQLServer::_column_info($self,'',$owner,'TABLE') if ($self->{is_sqlserver});
...
sub _column_attributes
{
	...
	return Ora2Pg::SQLServer::_column_attributes($self,'',$owner,'TABLE') if ($self->{is_sqlserver});
...
sub _encrypted_columns
{
	...
	return Ora2Pg::SQLServer::_encrypted_columns($self,'',$owner) if ($self->{is_sqlserver});
...
sub _unique_key
{
	...
	return Ora2Pg::SQLServer::_unique_key($self,$table,$owner,$type) if ($self->{is_sqlserver});
...
sub _foreign_key
{
	...
	return Ora2Pg::SQLServer::_foreign_key($self,$table,$owner) if ($self->{is_sqlserver});
...
sub _get_privilege
{
	...
	return Ora2Pg::SQLServer::_get_privilege($self) if ($self->{is_sqlserver});
...
sub _get_security_definer
{
	...
	return Ora2Pg::SQLServer::_get_security_definer($self, $type) if ($self->{is_sqlserver});
...
sub _get_indexes
{
	...
	return Ora2Pg::SQLServer::_get_indexes($self,$table,$owner) if ($self->{is_sqlserver});
...
sub _get_sequences
{
	...
	return Ora2Pg::SQLServer::_get_sequences($self) if ($self->{is_sqlserver});
...
sub _get_identities
{
	...
	return Ora2Pg::SQLServer::_get_identities($self) if ($self->{is_sqlserver});
...
sub _get_external_tables
{
	...
	return Ora2Pg::SQLServer::_get_external_tables($self) if ($self->{is_sqlserver});
...
sub _get_dblink
{
	...
	return Ora2Pg::SQLServer::_get_dblink($self) if ($self->{is_sqlserver});
...
sub _get_job
{
	...
	return Ora2Pg::SQLServer::_get_job($self) if ($self->{is_sqlserver});
...
sub _get_views
{
	...
	return Ora2Pg::SQLServer::_get_views($self) if ($self->{is_sqlserver});
...
sub _get_materialized_views
{
	...
	return Ora2Pg::SQLServer::_get_materialized_views($self) if ($self->{is_sqlserver});
...
sub _get_triggers
{
	...
	return Ora2Pg::SQLServer::_get_triggers($self) if ($self->{is_sqlserver});
...
sub _list_triggers
{
	...
	return Ora2Pg::SQLServer::_list_triggers($self) if ($self->{is_sqlserver});
...
sub _get_plsql_metadata
{
	...
	return Ora2Pg::SQLServer::_get_plsql_metadata($self, $owner) if ($self->{is_sqlserver});
...
sub _get_package_function_list
{
	...
	return Ora2Pg::SQLServer::_get_package_function_list($self, $owner) if ($self->{is_sqlserver});
...
sub _get_functions
{
	...
	return Ora2Pg::SQLServer::_get_functions($self) if ($self->{is_sqlserver});
...
sub _get_procedures
{
	...
	return Ora2Pg::MySQL::_get_functions($self) if ($self->{is_mysql});
	return Ora2Pg::SQLServer::_get_procedures($self) if ($self->{is_sqlserver});
...
sub _get_types
{
	...
	# return Ora2Pg::MySQL::_get_functions($self) if ($self->{is_mysql});
	return Ora2Pg::SQLServer::_get_types($self, $dbh, $name) if ($self->{is_sqlserver});
...
sub _table_info
{
	...
	return Ora2Pg::SQLServer::_table_info($self) if ($self->{is_sqlserver});
...
sub _global_temp_table_info
{
	...
	return Ora2Pg::SQLServer::_global_temp_table_info($self) if ($self->{is_sqlserver});
...
sub _get_tablespaces
{
	...
	return Ora2Pg::SQLServer::_get_tablespaces($self) if ($self->{is_sqlserver});
...
sub _list_tablespaces
{
	...
	return Ora2Pg::SQLServer::_list_tablespaces($self) if ($self->{is_sqlserver});
...
sub _get_partitions
{
	...
	return Ora2Pg::SQLServer::_get_partitions($self) if ($self->{is_sqlserver});
...
sub _get_subpartitions
{
	...
	return Ora2Pg::SQLServer::_get_subpartitions($self) if ($self->{is_sqlserver});
...
sub _get_synonyms
{
	...
	return Ora2Pg::SQLServer::_get_synonyms($self) if ($self->{is_sqlserver});
...
sub _get_partitions_list
{
	...
	return Ora2Pg::SQLServer::_get_partitions_list($self) if ($self->{is_sqlserver});
...
sub _get_partitioned_table
{
	...
	return Ora2Pg::SQLServer::_get_partitioned_table($self) if ($self->{is_sqlserver});
...
sub _get_subpartitioned_table
{
	...
	return Ora2Pg::SQLServer::_get_subpartitioned_table($self) if ($self->{is_sqlserver});
...
sub _convert_function
{
	...
	if ($fct_detail{hasreturn}) {
		...
		if ($nbout > 0) {
			...
			$func_return = " \nRETURNS$fct_detail{setof} $fct_detail{func_ret_type} \nAS \$body\$\n";
		}
	} else {
...
sub _extract_data
{
	...
	if ($self->{is_mysql}) {
		...
		# TODO: placeholder
		my %col_info = Ora2Pg::SQLServer::_column_info($self, $rname);
		foreach my $col (keys %{$col_info{$rname}}) {
			push(@{$self->{data_cols}{$table}}, $col);
		}
	} else {
...
sub _show_infos
{
	...
	if ($type eq 'SHOW_ENCODING') {
		if ($self->{is_mysql}) {
			...
			$self->logit("Current encoding settings that will be used by Ora2Pg:\n", 0);
			$self->logit("\tSQL Server database and client encoding: $self->{nls_lang}\n", 0);
			$self->logit("\tSQL Server collation encoding: $self->{nls_nchar}\n", 0);
			$self->logit("\tPostgreSQL CLIENT_ENCODING $self->{client_encoding}\n", 0);
			$self->logit("\tPerl output encoding '$self->{binmode}'\n", 0);
			my ($my_encoding, $my_client, $pg_encoding, $my_timestamp_format, $my_date_format) = &Ora2Pg::SQLServer::_get_encoding($self, $self->{dbh});
			$self->logit("Showing current SQL Server encoding and possible PostgreSQL client encoding:\n", 0);
			$self->logit("\tSQL Server database and client encoding: $my_encoding\n", 0);
			$self->logit("\tSQL Server collation encoding: $my_client\n", 0);
			$self->logit("\tPostgreSQL CLIENT_ENCODING: $pg_encoding\n", 0);
			$self->logit("\tSQL Server SQL mode: $self->{sqlserver_mode}\n", 0);
			$self->logit("\tSQL Server date format: $my_date_format\n", 0);
		...
		if ($self->{is_mysql}) {
			...
			$self->logit("Reporting MySQL Content...\n", 1);
		}
		elsif ($self->{is_sqlserver}) {
			$uncovered_score = 'Ora2Pg::PLSQL::UNCOVERED_SQLSERVER_SCORE';
			$self->logit("Reporting SQL Server Content...\n", 1);
		}
		else {
		...
		foreach my $typ (sort keys %objects) {
			...
			if ($typ eq 'INDEX') {
				...
				%largest_table = $self->_get_largest_tables() if ($self->{is_sqlserver});
				...
				if ($self->{is_sqlserver}){
					...
					$report_info{'Objects'}{$typ}{'detail'} .= "\L$t->{owner}.$t->{name}\E\n";
					}
				}
				else {
				...
				# TODO: correct for SQL Server
				foreach my $t (sort {$a cmp $b} keys %synonyms) {
					# print "SYNONYM:t=[$t], a=[$a], b=[$b]\n";
				...
				$def_fdw = 'tds_fdw' if ($self->{is_sqlserver}); # Write is not supported with this FDW
				...
				$report_info{'Objects'}{$typ}{'comment'} = "Jobs are not exported. You may set external cron job with them.";
				if ($self->{estimate_cost}) {
...
sub show_test_errors
{
	...
	my $rdbms = 'Oracle';
	$rdbms = 'MySQL' if ($self->{is_mysql});
	$rdbms = 'SQL Server' if ($self->{is_sqlserver});
	...
	if ($#errors >= 0) {
		...
		if ($self->{pg_dsn}) {
			print "OK, $rdbms and PostgreSQL have the same number of $lbl_type.\n";
		} else {
...
sub _table_row_count
{
	...
	$lbl    = 'SQLSERVER_DB' if ($self->{is_sqlserver});
...
sub _test_table
{
	...
	if ($self->{is_sqlserver}) {
		$indexes = Ora2Pg::SQLServer::_count_indexes($self, '', $self->{schema});
	}
...
AND pg_constraint.contype IN ('p')
 $schema_cond
	...
	if ($self->{is_sqlserver}) {
		# future placeholder
		# my %check_constraints = SQLServer::_check_constraint('',$self->{schema});
	}
	elsif (!$self->{is_mysql}) {
		print "\n";
...
WHERE e.relname = ? AND a.attnum > 0 AND NOT a.attisdropped
 $schema_cond
	...
	if ($self->{is_sqlserver}) {
		@seqs = Ora2Pg::SQLServer::_count_sequences($self);
	}
...
sub _get_version
{
	...
	return Ora2Pg::SQLServer::_get_version($self) if ($self->{is_sqlserver});
...
sub _get_database_size
{
	...
	return Ora2Pg::SQLServer::_get_database_size($self) if ($self->{is_sqlserver});
...
sub _get_objects
{
	...
	return Ora2Pg::SQLServer::_get_objects($self) if ($self->{is_sqlserver});
...
sub _list_all_funtions
{
	...
	return Ora2Pg::SQLServer::_list_all_funtions($self) if ($self->{is_sqlserver});
...
sub _schema_list
{
	...
	return Ora2Pg::SQLServer::_schema_list($self) if ($self->{is_sqlserver});
...
sub _table_exists
{
	...
	return Ora2Pg::SQLServer::_table_exists($self, $schema, $table) if ($self->{is_sqlserver});
...
sub _get_largest_tables
{
	...
	return Ora2Pg::SQLServer::_get_largest_tables($self) if ($self->{is_sqlserver});
...
sub limit_to_objects
{
	...
	for (my $i = 0; $i <= $#arr_type; $i++) {
		...
		if ($#{$self->{limited}{$arr_type[$i]}} >= 0) {
			...
			elsif ($self->{db_version} =~ /Release [89]/) {
				for (my $j = 0; $j <= $#{$self->{limited}{$arr_type[$i]}}; $j++) {
			...
			if($self->{is_sqlserver}){
				...
				for (my $j = 0; $j <= $#{$self->{excluded}{$arr_type[$i]}}; $j++) {				
					$str .= "'$self->{excluded}{$arr_type[$i]}->[$j]'" ;			
		...
		elsif (!$self->{is_mysql} &&  !$self->{is_sqlserver} && !$has_limitation && ($arr_type[$i] =~ /TABLE|SEQUENCE|VIEW|TRIGGER|TYPE|SYNONYM/)) {
			if ($self->{db_version} =~ /Release [89]/) {
				$str .= ' AND (';
...
sub _lookup_function
{
	...
	return Ora2Pg::MySQL::_lookup_function($self, $plsql, $pname) if ($self->{is_mysql});
	return Ora2Pg::SQLServer::_lookup_function($self, $plsql, $pname) if ($self->{is_sqlserver});
